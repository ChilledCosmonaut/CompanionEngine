...
namespace gl3::game {
    void ShootingMechanics::Shoot(engine::Graphics::Transform &transform) {
        ...
        entt::entity laserChild = entt::null;
        ...
        if (laserChild == entt::null)return;
        if (!registry.valid(laserChild)) return;
        if (!registry.any_of<Laser>(laserChild)) return;
        ...
        if (!registry.any_of<engine::Graphics::Transform>(laserChild)) return;
        ...
        if (victim != entt::null) {
            if (!registry.valid(laserChild)) return;
            if (!registry.any_of<engine::Graphics::Transform>(victim)) return;
            ...
            std::cout << "Hit enemy:" << std::to_string(static_cast<double>(victim)) << " with length:"
                      << std::to_string(laserLength) << std::endl;
        }
